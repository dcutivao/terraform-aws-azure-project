terraform {
  backend "s3" {
    ...
    key    = "prod/terraform.tfstate" # podemos indicar donde lo podemos alojar en s3, en este caso lo estamos alojando en una carpeta llamada prod que no necesita ser creada , pero si no existe la carpeta se creara automaticamente
    region = "us-east-1"
